...
#pragma warning restore SA1129 // Do not use default value type constructor
            ...
            try
            {
                ...
                {
                    var summary = await RunTestCollectionForUnspecifiedVersionAsync(ctxt.TestAssembly, testCollection, nonIdeTestCases, completedTestCaseIds, ctxt.CancellationTokenSource);
                ...
                foreach (var testCasesByTargetVersion in ideTestCases.GroupBy(GetVisualStudioVersionForTestCase))
                {
                    ...
                    for (var currentAttempt = 0; currentAttempt < testCasesByTargetVersion.Key.MaxAttempts; currentAttempt++)
                    {
                        ...
                        var summary = await RunTestCollectionForVersionAsync(visualStudioInstanceFactory, currentAttempt, currentInstance, ctxt, testCollection, currentTests, completedTestCaseIds, ctxt.CancellationTokenSource);
                        result.Aggregate(summary);
